...
// POST /api/campaigns       — create campaign (+ enqueue if schedule_type=now)
...
export const GET = withAuth(async (_req, ctx) => {
  const { data, error } = await ctx.supabase
    .from("campaigns")
    .select(`
      id, name, status, channels, schedule_type, scheduled_at,
...
export const POST = withAuth(async (req, ctx) => {
  ...
  // Resolve scheduled_at for AI scheduling (simple: next Tuesday 10:15)
  let scheduledAt = d.scheduledAt ? new Date(d.scheduledAt) : null;
  if (d.scheduleType === "ai") {
    scheduledAt = nextWeekday(2, 10, 15); // Tuesday 10:15
  }
  ...
  // Enqueue immediately if "now"
  if (d.scheduleType === "now") {
    await enqueueCampaign(campaign.id, ctx.tenantId, d.segmentId);
  }
...
// POST — manually trigger send for a scheduled campaign
// ─────────────────────────────────────────────────────────────
export const POST_SEND = withAuth(async (req, ctx) => {
  const id = req.url.split("/").at(-2)!;

  const { data: campaign } = await supabaseAdmin
    .from("campaigns")
    .select("id, status, segment_id, tenant_id")
    .eq("id", id)
    .eq("tenant_id", ctx.tenantId)
    .single();
  ...
  if (campaign.status !== "scheduled" && campaign.status !== "draft") {
    return json({ error: `Cannot send campaign in status: ${campaign.status}` }, 400);
  }
  ...
  await enqueueCampaign(id, ctx.tenantId, campaign.segment_id);
  return json({ ok: true });
});
...
// ─────────────────────────────────────────────────────────────
// SEND ENGINE
// In production this would be a separate worker (BullMQ / pg-boss).
// Here we process the queue in-process for simplicity.
// ─────────────────────────────────────────────────────────────
...
async function enqueueCampaign(campaignId: string, tenantId: string, segmentId?: string | null) {
  ...
  if (segmentId) {
    // Apply segment rules — simplified: just filter by segment tags for now
  ...
  const rows = subscribers.flatMap((s: any) => {
    const entries = [];
    // We'll use the campaign's channel config — simplified here
    entries.push({ campaign_id: campaignId, subscriber_id: s.id, channel: "whatsapp", status: "pending" });
    return entries;
  });
  ...
  // Kick off the worker (in prod: emit to queue)
...
async function processQueue(campaignId: string, tenantId: string) {
  ...
  while (true) {
    ...
    const { data: items } = await supabaseAdmin
      .from("send_queue")
      .select("id, subscriber_id, channel")
    ...
    for (const item of items) {
      try {
        if (item.channel === "whatsapp") {
          await sendWhatsApp(campaignId, tenantId, item.subscriber_id);
        } else {
          await sendEmail(campaignId, tenantId, item.subscriber_id);
        }
    ...
    // Small delay to respect Meta's rate limits (80 msg/sec on TIER_10K)
    await sleep(650);
  }
...
// ─────────────────────────────────────────────────────────────
// WhatsApp sender — calls Meta Cloud API
// ─────────────────────────────────────────────────────────────
async function sendWhatsApp(campaignId: string, tenantId: string, subscriberId: string) {
  ...
  const payload = {
    ...
    type:              "template",
    template: {
      name:       camp.wa_template_id,   // In real impl fetch template name from DB
      language:   { code: "tr" },
      components,
    },
  };
...
// ─────────────────────────────────────────────────────────────
// Email sender — uses SendGrid (swap for Postmark/Resend easily)
// ─────────────────────────────────────────────────────────────
